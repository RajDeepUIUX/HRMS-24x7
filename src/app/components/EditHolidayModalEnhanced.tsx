...
function Frame1000002499({ onClose }: { onClose: () => void }) {
  return (
    <div className="content-stretch flex items-center relative shrink-0 w-full">
      <p className="basis-0 font-['Inter:Semi_Bold',_sans-serif] font-semibold grow leading-[30px] min-h-px min-w-px not-italic relative shrink-0 text-[#1d2939] text-[20px]">Edit Holiday</p>
...
export default function EditHolidayModal({ isOpen, onClose, onSave, holiday }: EditHolidayModalProps) {
  ...
  // Update state when holiday changes
  useEffect(() => {
    if (holiday) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (name && date && type && description) {
      onSave({ name, date, type, description, recurring });
      onClose();
    }
  };
